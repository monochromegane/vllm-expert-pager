"""Check that gather_rows copies both weights and the scales into the slot. Needs a GPU."""
...
E, S = 8, 4
WORDS = (8192, 4096)  # w13, w2 (two chunks and one chunk with _BLOCK=4096)
SCALE_WORDS = (32, 16)
...
def test_gather_copies_rows_and_scales():
    ...
    # Expert 3 goes from RAM-tier row 7 (+base 2 = 9) to slot 1, expert 5 from
    # cache row 0 to slot 2. The remaining todo entries are empty.
    ...
    gather_rows(todo, dst_row, cache_row, src_row, 2, src, cache, dst, scale, scale_dst)
    torch.cuda.synchronize()

    for i in range(2):
        assert torch.equal(dst[i][1], src[i][9])
        ...
        assert not dst[i][0].any() and not dst[i][3].any()
